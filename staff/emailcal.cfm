...
<cfquery name="getEvents2" datasource="#request.dsn#">
	...
	Where g.g_id = #val(attributes.g_id)#	
	<cfif isDefined("attributes.v_id") AND val(attributes.v_id)>AND s.v_id = #attributes.v_id#</cfif>
	and visit_date between #CreateODBCDate(createDate(theyear,themonth,1))# 
...
<table width="150">
	...
	<tr style="background-color: ##eee;color:black;font: 9px verdana bold;">
		...
		<cfelse>
			<cfif ListFind(attributes.selectedlist,"#themonth#/#dayofmonth#/#theyear#") OR (caller.GetGV.g_permanent eq 1 AND  dateCompare(createdate(year(request.timezoneadjustednow),month(request.timezoneadjustednow),day(request.timezoneadjustednow)),createdate(theyear,themonth,dayofmonth)) lte 0)>
				<cfset startClassName = 'bgcolor="##66CC66"'>
			<cfelse>
				<cfset startClassName = 'bgcolor="##eeeeee"'>
			</cfif>
			 <!---  IF it was already selected --->
			   <td #startClassName#>#dayofmonth#</td>
		</cfif>
