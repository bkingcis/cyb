...
<cfquery name="getEvents2" datasource="#datasource#">
	...
	Where s.r_id = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#attributes.r_id#" />	
	and  s.h_id = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#session.h_id#" />
...
<table width="150">
	<tr><th colspan="7" style="background-color: #0071BE;color:#fff;padding-left:3px;"><cfoutput>#MonthAsString(themonth)#</cfoutput></th></tr>
	<tr style="background-color: #5996CD;color:#111;">
		...
		<cfif structkeyExists(stCalItems,'d'&theyear&themonth&dayofmonth)><!--- style="background-color:##f77;"  --->
			<cfif stCalItems['d'&theyear&themonth&dayofmonth] is 'visitor'>
				...
				<cfloop query="qOfqGetEvents">
					...
					<cfset datastring = datastring & '<li class="popBoxList"><a href="../popup/modifyschedule2.cfm?r_id=#r_id#&g_id=' & qOfqGetEvents.g_id & '&v_id=' & qOfqGetEvents.v_id &'">' & qOfqGetEvents.gname & appendToName & '</a>' & iif(qOfqGetEvents.insertedby_staff_id eq 0,"",DE('*')) & '</li>'>
					</cfif>
				</cfloop>
				...
				<cfloop query="qOfqGetEvents">
					<cfset datastring = datastring & '<li class="popBoxList"><a href="popup/modspecialevent.cfm?r_id=#r_id#&specialevent_id=' & qOfqGetEvents.specialevent_id & '">' & ucase(qOfqGetEvents.EventLabel) & '</a></li>'>
				</cfloop>
				<cfset datastring = datastring & '</ul>'>
				<cfelse>
					<cfset useTDclass = "calGuestBox">
				</cfif>
					<td class="#useTDclass#" onmouseover="this.className='calGuestBoxHover';" onmouseout="this.className='#useTDclass#'" data-toggle="modal" data-target="##baseModal" data-action="modifyschedule2" data-date="#themonth#/#dayofmonth#/#theyear#" id="guestcalbox_#theyear##themonth##dayofmonth#"> <!--- onclick="showDataPop('#urlEncodedFormat(dataString)#','#urlEncodedFormat(dateFormat(createDate(theyear,themonth,dayofmonth),"mmmm d, yyyy"))#');"--->#dayofmonth#</td>
			<cfelse>
				<cfquery dbtype="query" name="qOfqGetEvents">
					select * from getEvents
					WHERE dayofmonthdata = #dayofmonth#
				...
				<cfset datastring = '<ul style="list-style-type:none;margin:0px;padding:0px;">'>
				<cfloop query="qOfqGetEvents">
					<cfset datastring = datastring & '<li class="popBoxList"><a href="popup/modspecialevent.cfm?specialevent_id=' & qOfqGetEvents.specialevent_id & '">' & ucase(qOfqGetEvents.EventLabel) & '</a></li>'>
				</cfloop>
				<td class="calEventBox" onmouseover="this.className='calEventBoxHover';" onmouseout="this.className='calEventBox'" data-toggle="modal" data-target="##baseModal" data-action="modspecialevent" data-id="#qOfqGetEvents.specialevent_id#" data-date="#themonth#/#dayofmonth#/#theyear#"> <!--- onclick="showDataPop('#urlEncodedFormat(dataString)#','#urlEncodedFormat(dateFormat(createDate(theyear,themonth,dayofmonth),"mmmm d, yyyy"))#');"--->#dayofmonth#</td>
			</cfif>
		<cfelse>
...
<!--- <cfoutput>GetEvents: #GetEvents.recordcount# GetEvents2: #GetEvents2.recordcount#</cfoutput> --->
