...
<html lang="en">
  <head>
    ...
    <title>Returning User - Cybatrol</title>
  ...
  <div class="nav" style="background-image: url('/img/logo.png');
	...
	<div class="jumbotron">	 
	<form class="form" role="form" id="LoginFrm" action="/staff/login.cfm" method="post">
		<p></p>
		<fieldset>
		<cfoutput><legend>#request.page_title#</legend></cfoutput>
		  ...
		  <div class="form-group row">
			...
			<br />
			<a href="forgot.cfm?login_type=<cfoutput>#login_type#</cfoutput>" data-target="#baseModal" data-action="forgot">Forgot your password?</a>
			</div>
		  </div>
		<fieldset/>	
	  <div class="form-group">
		<div class="col-sm-offset-2 col-sm-4 col-md-offset-2 col-md-4">
		  <button id="btn_continue" type="button" class="btn btn-primary" data-toggle="modal" data-target="#baseModal" data-action="login">LOGIN</button>
		</div>
	  </div>
	</form>
	</div><!-- /.jumbotron -->
			...
			$(function() {
				...
				$('#baseModal').on('show.bs.modal', function (event) {
				  ...
				  var formdata = {
					...
					<cfoutput>var theUrl = "#request.login_processor#";</cfoutput>
					  $.ajax({
						  method: "POST",
						  url: theUrl,
						  data: formdata
						...
						.done(function( result ) {	
						  ...
						  <cfoutput>modal.find('.modal-title').text('#request.page_title#');</cfoutput>
						})
					   .fail(function() {
