<cftry>
<cfif isDefined("form.pass1") and isDefined("form.pass2")>
	<cfinclude template="act_resetAction.cfm">
<cfelse>
	<cfinclude template="act_validatetoken.cfm">
</cfif>
...
<html lang="en">
  <head>
    ...
    <![endif]-->
		<style>
			...
			.hilite {	border-bottom: 1px solid silver; }
  ...
  <body class="signup">
    <div class="container">
		...
		<cfif isDefined("form.pass1") and isDefined("form.pass2")>
				<cfinclude template="inc_resetResults.cfm">
		<cfelse>
				<cfinclude template="inc_resetForm.cfm">
		</cfif>
			</div><!-- /.jumbotron -->
    </div><!-- /.container -->
	...
	<script>
			$(function() {
				...
				$('#btn_continue').on( "click", function() {  
				  if (!$('input[name=pass1]').val().length) { 
				  	alert('Please supply an password.');
					 return false;
				  }
				});
			});
